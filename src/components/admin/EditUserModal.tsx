...
export default function EditUserModal({ user, onClose, onSave }: EditUserModalProps) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl z-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4 p-3 bg-blue-50 rounded-lg">
                <input 
                  type="checkbox" 
                  checked={data.isTop} 
                  onChange={(e) => setData({...data, isTop: e.target.checked})} 
                  className="w-4 h-4 text-blue-600"
                />
                <label className="flex-1 font-medium text-blue-900">TOP</label>
                <input 
                  type="date" 
